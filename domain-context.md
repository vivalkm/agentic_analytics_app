...
- **Rewire**: A non-core product (acquired company). All Rewire transactions are sent from Israel (ISR), and all ISR send volume is Rewire. Simplest filter: `corridor_key LIKE 'ISR-%'`. Alternative on `fpa.transaction_economics`: `txn_is_migrated_rewire = TRUE AND transaction_remittance_distro_id = 'app_id_ra_ril'`. Note: `txn_is_core = TRUE` already excludes Rewire. **Data caveat**: Rewire transaction data in Lakehouse is incomplete before 2026-01-01. If the user asks about Rewire data prior to January 2026, warn them that results may be incomplete or missing.
